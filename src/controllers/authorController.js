...
try {
	  ...
	  const { fname, lname,title, password }=data
	
	  if (!validator.isValidName(fname)) return res.status(400).send({ msg: "fname is mandatory" });
	
	  if (!validator.isValidName(lname)) return res.status(400).send({ msg: "lname in mandatory" }); 
	  ...
	  if (!validator.isValidPassword(password))
	    return res.status(404).send({ msg: "password is mandatory& should contain Min 8 character and 1 Special Symbol" });
	
	   if(!validator.isValidEmail(email))  return res.status(400).send({msg:"email in mandatory"})
